...
if Base is not object:
    class User(Base):
        ...
        is_admin = sqlalchemy.Column(sqlalchemy.Boolean, default=False) # New field for admin users
...
# --- PYDANTIC SCHEMAS ---
# (UserCreate, PasswordReset, Token, TokenData remain the same)
...
class UserInDB(BaseModel):
    ...
    name: str
    phone_number: str
    is_admin: bool # Include admin status
    class Config:
...
# --- SECURITY & AUTH ---
# (pwd_context, verify_password, get_password_hash, create_access_token are the same)
...
# --- NEW DEPENDENCY FOR ADMINS ONLY ---
...
def get_user_activity(user_id: int, db: Session = Depends(get_db)):
    """
    (Admin only) Get a specific user and all their attendance records.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
        
    # Eagerly load related event details to avoid extra queries
    ...
    # Manually map to the Pydantic schema
    records_out = [
        AttendanceRecord(
            id=rec.id,
